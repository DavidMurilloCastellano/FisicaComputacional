...
#define pMC 2e4 //Número de pasos de Monte-Carlo que se dan para calcular cada promedio de magnitudes
...
int main (void)
{
    double T, h, p, en, en2, sMag, sMag2, sE, sE2, sE4, mMag, mE, mE2, varMag, varE, varE2, aux, den;
    ...
    for(k=0; k<nT; k++)
    {
        ...
        mMag=sMag/den; mE=sE/den; mE2=sE2/den; aux=mE*mE;
        varMag=sMag2/den-mMag*mMag; varE=mE2-aux; varE2=sE4/den-mE2*mE2; 
        ...
        fichO << ", " << mMag/M << ", " << 1.96*sqrt(varMag/den)/M << ", "; //Cálculo de la magnetización promedio
        fichO << mE/(2*N) << ", " << 1.96*sqrt(varE/den)/(2*N) << ", "; //Cálculo de la energía media
        fichO << varE/(M*T) << ", " << 1.96*sqrt((varE2+4*aux*varE)/den)/(M*T) << endl;//Cálculo del calor específico 
