...
file_fit2="fit_long_corr-N="+N+".txt"
...
for temp in T:
    c=next(color)
    y=frames_data[i][1][:32]
    plt.errorbar(x,y,yerr=frames_data[i][2][:32],fmt='none',capsize=2,c=c)
    optimizedParameters, pcov = opt.curve_fit(func, x, y, sigma=frames_data[i][2][:32],absolute_sigma=True)
    plt.plot(z, func(z, *optimizedParameters),c=c)
    plt.xlabel("Distancia, $i$")
    plt.ylabel("$f(i)$")
    A.append(optimizedParameters[2])
    C.append(1.96*np.sqrt(pcov[2][2]))
    i=i+1
plt.title("N="+N)
box = ax.get_position()
ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
...
with open(file_fit, 'w') as f:
    for j in range(len(T)):
        f.write(str(t[j])+', '+str(A[j])+', '+str(C[j])+'\n')
...
if(ajuste):
    ...
    z=np.linspace(0,x[-1],100)
    fig3=plt.figure()
    plt.errorbar(x,y,yerr=d_y,fmt='none',capsize=2,label="Datos")
    optimizedParameters, pcov = opt.curve_fit(lineal, x, y, sigma=d_y,absolute_sigma=True)
    plt.plot(z, lineal(z, *optimizedParameters),label="Ajuste",c='m')
    plt.xlabel("ln$(T_c-T)$")
    plt.ylabel(r"ln$\xi$")
    ...
    with open(file_fit2, 'w') as f:
        f.write("m="+str(optimizedParameters[1])+"$\pm$"+str(np.sqrt(pcov[1][1])))
